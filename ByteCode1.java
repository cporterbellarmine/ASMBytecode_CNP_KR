...
public class ByteCode1 {
	...
	public static void main(String args[]) {
	    ...
	    cw.visit(Opcodes.V11, Opcodes.ACC_PUBLIC,"Multiply", null, "java/lang/Object",null); //creating my main constructor for the file
...

	    {
	        ...
	        /**
             * This block of code takes an initial double and multiplies it by a second double.
             */
	        ...
	        mv.visitVarInsn(Opcodes.DLOAD,1); // loads the first double to the stack
	        mv.visitVarInsn(Opcodes.DLOAD,3); // loads the second double to the stack
	        mv.visitInsn(Opcodes.DMUL); // multiplies the values and returns the product
	        mv.visitVarInsn(Opcodes.DSTORE,5); // stores the product
	        mv.visitFieldInsn(Opcodes.GETSTATIC, "java/lang/System", "out", "Ljava/io/PrintStream;"); // sets up the stream to print the next line
	        mv.visitVarInsn(Opcodes.DLOAD, 5); // loads the product to be printed
	        mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, "java/io/PrintStream", "println", "(D)V", false); // prints the quotient
	        
	        /**
             * This block of code takes an initial long and multiplies it by a second long.
             */
	        ...
	        mv.visitInsn(Opcodes.LMUL); // multiplies the values and returns the product
	        mv.visitVarInsn(Opcodes.LSTORE,11); // stores the product to the stack
	        mv.visitFieldInsn(Opcodes.GETSTATIC, "java/lang/System", "out", "Ljava/io/PrintStream;"); // sets up the stream to print the next line
	        mv.visitVarInsn(Opcodes.LLOAD, 11); // loads the product to be printed
	        mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, "java/io/PrintStream", "println", "(J)V", false); // prints the product
	        
	        /**
             * This block of code takes an initial integer and multiplies it by a second integer.
             */
	        mv.visitLdcInsn((Integer)10); // initializes the first integer
	        mv.visitVarInsn(Opcodes.ISTORE,13); // store the first integer in the stack
	        mv.visitLdcInsn((Integer)5); // initializes the second integer
	        mv.visitVarInsn(Opcodes.ISTORE,15); // stores the second integer in the stack
	        mv.visitVarInsn(Opcodes.ILOAD,13); // loads the first integer
	        mv.visitVarInsn(Opcodes.ILOAD,15); // loads the second integer
	        mv.visitInsn(Opcodes.IMUL); // multiplies the two values and returns the product
	        mv.visitVarInsn(Opcodes.ISTORE,17); // stores the product in the stack
	        mv.visitFieldInsn(Opcodes.GETSTATIC, "java/lang/System", "out", "Ljava/io/PrintStream;"); // sets up the stream to print the next line
	        mv.visitVarInsn(Opcodes.ILOAD, 17); // loads the product to be printed
	        mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, "java/io/PrintStream", "println", "(I)V", false); // prints the product
        ...
        writeFile(b,"Multiply.class"); //Writes the byte array to class file
