...
public class ByteCode2 {
    ...
    cw.visit(Opcodes.V11, Opcodes.ACC_PUBLIC,"Subtract", null, "java/lang/Object",null); //creating my main constructor for the file
...

    {
        ...
        /**
         * This block of code takes an initial double and subtracts a second double from it.
         */
        mv.visitLdcInsn((Double)173.43); // initializes the first double
        mv.visitVarInsn(Opcodes.DSTORE,1); // stores the first double in the stack
        mv.visitLdcInsn((Double)45.56); // initializes the second double
        mv.visitVarInsn(Opcodes.DSTORE,3); // stores the second double in the stack
        mv.visitVarInsn(Opcodes.DLOAD,1); // loads the first double
        mv.visitVarInsn(Opcodes.DLOAD,3); // loads the second double
        mv.visitInsn(Opcodes.DSUB); // subtracts the second double from the first and returns the difference
        mv.visitVarInsn(Opcodes.DSTORE,5); // stores the value given from subtraction on the stack
        mv.visitFieldInsn(Opcodes.GETSTATIC, "java/lang/System", "out", "Ljava/io/PrintStream;"); // sets up the stream for the next line to be printed
        mv.visitVarInsn(Opcodes.DLOAD, 5); // loads the difference to be printed
        mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, "java/io/PrintStream", "println", "(D)V", false); // prints the difference
        ...
        mv.visitLdcInsn((long)1234567890); // initializes the second long
        ...
        mv.visitInsn(Opcodes.LSUB); // subtracts the second long from the first one and returns the difference
        mv.visitVarInsn(Opcodes.LSTORE,11); // stores the value given by subtraction on the stack
        mv.visitFieldInsn(Opcodes.GETSTATIC, "java/lang/System", "out", "Ljava/io/PrintStream;"); // sets up the stream to print the next line
        mv.visitVarInsn(Opcodes.LLOAD, 11); // loads the difference to be printed
        mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, "java/io/PrintStream", "println", "(J)V", false); // prints the difference
        
        /**
         * This block of code takes an initial double and subtracts a second double from it.
         */
        mv.visitLdcInsn((Integer)10); // initializes the first integer
        mv.visitVarInsn(Opcodes.ISTORE,13); // stores the first integer on the stack
        mv.visitLdcInsn((Integer)5); // initializes the second integer 
        mv.visitVarInsn(Opcodes.ISTORE,15); // stores the second integer on the stack
        mv.visitVarInsn(Opcodes.ILOAD,13); // loads the first integer
        mv.visitVarInsn(Opcodes.ILOAD,15); // loads the second integer
        mv.visitInsn(Opcodes.ISUB); // subtracts the second integer from the first
        mv.visitVarInsn(Opcodes.ISTORE,17); // stores the difference on the stack
        mv.visitFieldInsn(Opcodes.GETSTATIC, "java/lang/System", "out", "Ljava/io/PrintStream;"); // sets up the stream to print the next line
        mv.visitVarInsn(Opcodes.ILOAD, 17); // loads the difference to be printed
        mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, "java/io/PrintStream", "println", "(I)V", false); // prints the difference
    ...
    writeFile(b,"Subtract.class"); //Writes the byte array to class file
